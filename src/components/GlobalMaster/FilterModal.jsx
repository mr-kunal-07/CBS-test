...
const FilterModal = ({ masterKey, initialFilters = {}, onClose, onApply }) => {
  ...
  const handleClear = () => {
    const cleared = {};
    config.filterFields.forEach((f) => { cleared[f.key] = ""; });
    setFilters(cleared);
    onApply(cleared);
    onClose();
  };
      ...
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900">
        <div className="flex items-start justify-between border-b border-slate-100 pb-4 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Filter</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Apply filters to narrow down records</p>
            </div>
          </div>
        ...
        <div className="mt-4 space-y-4">
          {config.filterFields.map((field) => (
            <div key={field.key}>
              <label className="mb-1.5 block text-sm font-medium text-[#1F2858] dark:text-slate-100">{field.label}</label>
              <input
                type="text"
        ...
        <div className="mt-6 flex items-center justify-end gap-3 border-t border-slate-100 pt-4 dark:border-slate-800">
          <button type="button" onClick={handleClear} className="rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
            Clear
          </button>
          <button type="button" onClick={handleApply} className="rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-primary-700">
            Apply Filter
          </button>
        </div>
      </div>
